    ...
    def __init__(self, parent, rows, columns):
        ...
        for row in range(self.rows):
            for column in range(self.columns):
                ...
                enterMaxMonth = tk.Label(self, text="Enter the maximum number of vacation days for any given month")
                ...
                enterMaxYear = tk.Label(self, text="Enter the maximum number of vacation days allotted for the year")
...
for m in range(8):
    if m == 0:
        currentYear = innerList[m]
    if m == 1:
        reliabilityLevel = innerList[m]
    if m == 2:
        dailyUpperBoundVacationAllotted = innerList[m]
    if m == 3:
        dailyLowerBoundVacationAllotted = innerList[m]
    if m == 4:
        userMonthlyUpperBoundVacationAllotted = innerList[m]
    if m == 5:
        monthlyLowerBoundVacationAllotted = innerList[m]
    if m == 6:
        userAnnualUpperBoundVacationAllotted = innerList[m]
    if m == 7:
        annualLowerBoundVacationAllotted = innerList[m]
...
monthlyLineHolders = [] 
for m in range(12):
       monthlyLineHolders.append(innerList2[m]) 
       

monthlyCrewScheduled = [] 
for m in ListAppend1:
       monthlyCrewScheduled.append(innerList2[m]) 


averageMonthlyExpectedAbsences = []
for m in ListAppend2:
       averageMonthlyExpectedAbsences.append(innerList2[m]) 


varOfMonthlyExpectedAbsences = []
for m in ListAppend3:
       varOfMonthlyExpectedAbsences.append(innerList2[m]) 
...
#-----Calculating the expected absences for each month based on the average absences, std. dev. of absences, and the reliability level-----
...
for m in Months:
       n.addConstr(vacationVariable[m].sum() <= userMonthlyUpperBoundVacationAllotted, name = "userMonthlyUpperBoundVacationAllottedForMonth" + str(m))
for m in Months:
...
n.addConstr(totalObjectiveValue <= userAnnualUpperBoundVacationAllotted, name = "userTotannualUpperBoundVacationAllotted") 
...
#-----Creating files with the deatils of the formulation and optimization for further inspection/review-----
n.write('sabre2.sol')
n.write("out.lp")
n.write("outs.mps")
